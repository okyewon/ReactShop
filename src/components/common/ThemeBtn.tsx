...
export default function ThemeBtn() {
  const isDark = useRecoilValue(themeIsDark);

  const toggleTheme = () => {
    const root = document.documentElement;
    const dark = CONSTANTS.THEME.DARK;
    ...
    if (darkMode) {
      localStorage.setItem(CONSTANTS.LOCAL_STORAGE_KEY.THEME, light);

      root.classList.remove(dark);
      root.dataset.theme = light;
    } else {
      localStorage.setItem(CONSTANTS.LOCAL_STORAGE_KEY.THEME, dark);
  ...
  return (
    <label className="swap swap-rotate mr-2 sm:mr-4">
      <input type="checkbox" className="js-theme" onChange={toggleTheme} />
      <svg
        className={`${isDark ? "swap-off" : "swap-on"} fill-white w-7 h-7`}
      ...
      <svg
        className={`${isDark ? "swap-on" : "swap-off"} fill-black w-7 h-7`}
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
      >
